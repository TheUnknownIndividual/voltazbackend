    ...
    public sealed class ContentAiGenerationProcessor
    {
        ...
        private async Task<AiGeneration> GenerateAsync(ContentAiGenerationStartRequest request, CancellationToken ct)
        {
            ...
            var briefingBlock = request.SourceArticle is { } source
                ...
                Before writing, judge whether this source article is genuinely relevant to a solar installation company's readers. It is relevant ONLY if its main subject is solar energy or other renewable power generation, energy storage, grid connection/net metering, incentives, tariffs, tenders, permits or regulations that directly affect them, or a concrete renewable project or capacity announcement. It is NOT relevant if it is a general ministry news item (meetings, councils, visits, appointments, ceremonies, cooperation or standardization talks), covers oil, gas, coal or conventional power, or only mentions energy or renewables in passing. When in doubt, set isRelevant to false. Explain briefly in relevanceReason. If isRelevant is false, you may return minimal placeholder text (e.g. just echo the original title) for every language's fields instead of writing a full article -- do not spend effort drafting an article you've judged irrelevant.
                """
                : """
