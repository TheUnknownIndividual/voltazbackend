...
namespace Volt.API.Controllers
{
    ...
    [ApiController]
    public sealed class PartnershipRequestsController : CustomBaseController
    {
        private readonly IPartnershipRequestService _service;

        public PartnershipRequestsController(IPartnershipRequestService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Create([FromBody] PartnershipRequestCreateRequest request, CancellationToken ct)
            => CreateActionResult(await _service.CreateAsync(request, ct));

        [Authorize]
        [HttpPut("{id:int}")]
        public async Task<IActionResult> Update(int id, [FromBody] PartnershipRequestUpdateRequest request, CancellationToken ct)
            => CreateActionResult(await _service.UpdateAsync(id, request, ct));

        [Authorize]
        [HttpPatch("status")]
        public async Task<IActionResult> UpdateStatus(int id, [FromBody] PartnershipRequestStatusUpdateRequest request, CancellationToken ct)
